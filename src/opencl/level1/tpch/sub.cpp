...
void SubApp::RunCPUReference(double &t)
{
	//Timer timer;
	//timer.reset();
	unsigned int sz;
...
int SubApp::SetKernel()
{
	...
	try
	{
		mKernel1 = cl::Kernel(program, "Subtraction", &err);
	}
	catch (cl::Error e)
	{
...
int SubApp::RunKernel()
{
	...
	//timer.stop();
	double writing = inst->Stop(start,"Kernel execution")/10;
